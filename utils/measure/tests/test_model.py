...
def test_write_model_json_merges_extra_measure_settings(tmp_path: Path) -> None:
    write_model_json(
        tmp_path,
        standby_power=0.3,
        name="Test device",
        measure_device="Test meter",
        parameters=base_parameters(),
        extra_measure_settings={"MERGED_FROM": ["left", "right"], "EXTENDED_FROM": "seed-1"},
    )

    model = json.loads((tmp_path / "model.json").read_text(encoding="utf-8"))
    assert model["measure_settings"]["MERGED_FROM"] == ["left", "right"]
    assert model["measure_settings"]["EXTENDED_FROM"] == "seed-1"
    assert model["measure_settings"]["SAMPLE_COUNT"] == 1

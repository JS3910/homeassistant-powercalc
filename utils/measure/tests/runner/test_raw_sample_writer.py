...
def test_write_records_primary_power_without_a_composite_reading(tmp_path: Path) -> None:
    path = tmp_path / "brightness.raw.jsonl"
    writer = RawSampleWriter(str(path))

    writer.write(mode=LutMode.BRIGHTNESS, variation=Variation(1), reading=None, power=2.4)
    writer.close()

    row = json.loads(path.read_text().splitlines()[0])
    assert row["primary"]["power"] == 2.4
    assert "witnesses" not in row

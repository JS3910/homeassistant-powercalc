...
def test_get_power_falls_back_to_the_newest_accepted_reading_within_stale_after() -> None:
    ...
    clock.now = 1002.5
    with pytest.raises(WaitingForMeterError, match="camera silent, reconnecting"):
        meter.get_power()
    assert frames.recovered == 1
...
def test_get_power_waits_while_new_frames_are_unreadable() -> None:
    clock = Clock()
    engine = scripted_engine("Power4.60w", "V233.0vC0.055A", "PF0.354")
    meter, frames = make_meter(engine, clock, window_seconds=1.5, stale_after_seconds=5.0)
    meter.process(frame(1, 997.0))
    engine.recognitions[rect_of(VI_BOX)] = "V235.6vComm021A"
    meter.process(frame(2, 1002.0))
    clock.now = 1002.5

    with pytest.raises(WaitingForMeterError, match="waiting for a readable frame; last frame: current unreadable"):
        meter.get_power()
    assert frames.recovered == 0
...
def test_get_power_before_any_frame_or_acceptance_explains_why() -> None:
    ...
    with pytest.raises(WaitingForMeterError, match="waiting for the first camera frame"):
        meter.get_power()
    with pytest.raises(PowerMeterError, match="no frame processed yet"):
        meter.diagnostic_sample()

    meter.process(frame(1, 1000.0))
    with pytest.raises(WaitingForMeterError, match="waiting for a readable frame; last frame: power unreadable"):
        meter.get_power()
    assert frames.recovered == 0
...
def test_recover_forgets_the_location_and_stale_accepts_and_reconnects() -> None:
    clock = Clock()
    engine = scripted_engine("Power4.60w", "V233.0vC0.055A", "PF0.354")
    meter, frames = make_meter(engine, clock)
    meter.process(frame(1, 1000.0))
    assert meter.get_power().power == 4.6
    assert meter._reader.location is not None  # noqa: SLF001

    meter.recover()

    assert frames.recovered == 1
    assert meter._reader.location is None  # noqa: SLF001
    with pytest.raises(WaitingForMeterError, match="waiting for camera after reconnect"):
        meter.get_power()

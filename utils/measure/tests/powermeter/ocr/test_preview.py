...
from measure.powermeter.ocr.preview import PreviewBindError, PreviewServer, annotate
...
def test_bind_failure_names_the_listen_address(monkeypatch: pytest.MonkeyPatch) -> None:
    def fail(_self: object, _address: tuple[str, int], _preview: object) -> None:
        raise OSError(98, "Address already in use")

    monkeypatch.setattr("measure.powermeter.ocr.preview._Server.__init__", fail)
    with pytest.raises(PreviewBindError, match=r"127\.0\.0\.1:8765.*aiming-preview HTTP server") as caught:
        PreviewServer("127.0.0.1", 8765)

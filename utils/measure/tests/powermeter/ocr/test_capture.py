...
def test_recover_reopens_the_capture_without_waiting_for_backoff() -> None:
    first = FakeCapture(itertools.repeat(blank_frame()), clock=Clock())
    second = FakeCapture(iter([blank_frame()]))
    opener = Opener([first, second])
    source = FrameSource("0", clock=Clock(), open_capture=opener, reconnect_delay=5, max_reconnect_delay=5).start()
    try:
        assert source.wait_for_frame(0, timeout=5.0) is not None
        source.recover()
        wait_until(lambda: first.released)
        wait_until(lambda: len(opener.opened_with) >= 2)
        assert opener.opened_with == [0, 0]
    finally:
        source.close()

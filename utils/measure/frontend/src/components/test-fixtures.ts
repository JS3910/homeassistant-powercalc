...
export const lightDefinition: MeasureDefinition = {
  ...
  parameters: [
    ...
    modeParameter("ct_mired_divisions", "Color temperature divisions"),
    modeParameter("hs_bri_steps", "HS brightness step"),
    modeParameter("hs_hue_divisions", "HS hue divisions"),
  ...
  fields: [
    ...
    { name: "light_entity_id", role: "controller", label: "Light", plural_label: "Lights", control: "entity", required: true, multiple: true, entity_domains: ["light"], options: [] },
    {
      ...
      options: [
        ...
        { value: "color_temp", label: "Color temperature", enables: ["ct_bri_steps", "ct_mired_divisions"] },
        { value: "hs", label: "Hue & saturation", enables: ["hs_bri_steps", "hs_hue_divisions", "hs_sat_steps"] },
        { value: "effect", label: "Effect", enables: ["effect_bri_steps", "measure_time_effect_min", "measure_time_effect"] },
      ],
    },
    { name: "multiple_light_count", role: "attribute", label: "Number of lights", control: "number", required: true, options: [], default: 1, minimum: 1, maximum: 100, step: "1", derived_from: "light_entity_id" },
    { name: "rated_power_w", role: "attribute", label: "Rated power per light (W)", control: "number", required: false, options: [], minimum: 0, step: "0.1" },
  ],

...
import { DEFAULT_SHELLY_USERNAME, POWER_METER_LIST, meterFor, settingsFromForm } from "../../power-meter/registry";
...
const WITNESS_METER_TYPES: PowerMeterType[] = POWER_METER_LIST.map((meter) => meter.type).filter(
  (type) => type !== "dummy",
);
...
/** How long to wait after the last relevant keystroke before (re)starting the camera
 * preview -- avoids hammering the endpoint (which rebuilds and briefly holds the meter)
 * on every character typed into a source URL field. */
const OCR_PREVIEW_DEBOUNCE_MS = 800;
...
const SETTINGS_SECTIONS: SettingsSectionDescriptor[] = [
  ...
  },
  {
    ...
    icon: () =>
      icon(
        svg`<path d="M9 19c-4.2 1.2-4.2-2-6-2.4M15 22v-3.5c0-1 .1-1.4-.5-2 2.8-.3 5.5-1.4 5.5-6a4.7 4.7 0 0 0-1.3-3.3 4.4 4.4 0 0 0-.1-3.2s-1-.3-3.4 1.3a11.8 11.8 0 0 0-6.2 0C6.6 3.7 5.6 4 5.6 4a4.4 4.4 0 0 0-.1 3.2A4.7 4.7 0 0 0 4.2 10.5c0 4.6 2.7 5.7 5.5 6-.6.6-.6 1.2-.5 2V22"></path>`,
      ),
  },
];
...
export class SettingsView extends LitElement {
  ...
  /** Set once a camera-aiming preview is running for the currently configured OCR
   * meter(s); `null` while nothing is configured to preview. See `startMeterPreview`. */
  @property({ type: String })
  ...
  private ocrPreviewDebounce?: ReturnType<typeof setTimeout>;
  ...
  static readonly styles = [
    ...
    css`
      ...
      .witness-position legend {
        padding: 0 0.3rem;
        color: var(--muted);
        font-size: 0.76rem;
        font-weight: 650;
      }
      .witness-position-option {
        display: flex;
        gap: 0.55rem;
        align-items: flex-start;
        cursor: pointer;
      }
      .witness-position-option input {
        margin-top: 0.2rem;
        flex: none;
        width: auto;
        min-height: auto;
      ...
      .witness-position-option > span {
        flex: 1;
        min-width: 0;
      }
      .witness-position-option strong {
        display: block;
        font-size: 0.82rem;
      }
      .witness-position-option .field-hint {
        margin-top: 0.1rem;
      }
      .test-row {
        display: grid;
      ...
      .discovery-header strong {
        color: var(--ink);
        font-size: 0.82rem;
      }
      .discovery-header button {
        min-height: 36px;
  ...
  willUpdate(changedProperties: PropertyValues<this>) {
    if (changedProperties.has("settings")) {
      ...
      if (!this.witnessesInitialized && this.settings) {
        ...
        // The camera needs to be aimed as soon as it's in play at all -- including a
        // session resumed/opened straight into an already-OCR-configured meter, not just
        // one just switched to OCR in this form.
        this.scheduleOcrPreview();
      }
    }
  ...
  render() {
    ...
    const contributorGithub =
      this.contributorGithubValue ?? (this.settings?.default_contributor_github || connectedGithubUsername || "");
        ...
        <form ${ref(this.form)} @submit=${this.submit}>
          <div class="settings-layout">
            ...
            <section
              ...
              ?hidden=${this.activeSection !== "power_meter"}
              aria-labelledby="power-meter-title"
            >
              ...
              <div class="section-fields">
                ...
                ${this.measureDevicesError
                  ? html`<small class="field-hint error" role="status"
                      >Library suggestions are unavailable; manual entry still works.</small
                    >`
                  : nothing}
                <label>
                ...
                ${optionSelect(
                  "power_meter",
                  "Type",
                  POWER_METER_LIST.map((meter) => ({ value: meter.type, label: meter.label })),
                  {
                    selected: powerMeter,
                    required: true,
                ...
                ${descriptor.qualityNote
                  ? html`<p class="quality-requirements">${descriptor.qualityNote}</p>`
                  : nothing}
              ...
              <div class="section-fields witnesses">
                ...
                <p class="muted">
                  Optional secondary meters read alongside the primary. Each reading is checked against the primary's
                  within its tolerance before being accepted, which catches a misread or a meter that has drifted or
                  stopped updating.
                </p>
            ...
            <section
              class="settings-section"
              ?hidden=${this.activeSection !== "profile"}
              aria-labelledby="profile-metadata-title"
            >
              <h3 id="profile-metadata-title">Profile metadata</h3>
              <p class="muted">
                Set contributor details once. They are prefilled when preparing every profile and remain editable there.
              </p>
              <div class="section-fields">
                <label>
                  ...
                  <input
                    ...
                    .value=${this.settings?.default_contributor_name ?? ""}
                    autocomplete="name"
                  />
                </label>
                <label>
                ...
                <label>
                  ...
                  <input
                    name="default_contributor_email"
                    type="email"
                    .value=${this.settings?.default_contributor_email ?? ""}
                    autocomplete="email"
                  />
                </label>
              </div>
            ...
            <section
              class="settings-section"
              ?hidden=${this.activeSection !== "measure_tuning"}
              aria-labelledby="measure-tuning-title"
            >
              <h3 id="measure-tuning-title">Measure tuning</h3>
              <p class="muted">
                Set reusable timing, sampling, and recovery defaults. Relevant values can still be adjusted per
                measurement.
              </p>
              ${this.capabilities?.developer_mode
                ? html`
                    <div class="developer-option">
                      <label class="check">
                        <input
                          type="checkbox"
                          name="fast_test_mode"
                          .checked=${this.settings?.fast_test_mode ?? false}
                        />
                        <span>
                          <strong>Fast test mode</strong><br />
                          Synthetic light, fan, and charging workflows only. Skips waits and reduces measurement points
                          so the output is not valid for contribution or real use.
                        </span>
                      </label>
                    </div>
                  `
              ...
              <div class="grid">
                ${this.numberField(
                  ...
                  120,
                  "0.1",
                  "Wait after changing a device and between readings.",
                )}
                ${this.numberField(
                  "sample_count",
                  "Samples per point",
                  ...
                  100,
                  "1",
                  "More samples reduce noise but increase measurement time.",
                )}
                ${this.numberField(
                  "sleep_time_sample",
                  "Time between samples (seconds)",
                  defaults.sleep_time_sample,
                  0,
                  120,
                  "0.1",
                  "Used when taking more than one sample per point.",
                )}
                ${this.numberField(
                  "max_retries",
                  "Power meter retries",
                  ...
                  100,
                  "1",
                  "Consecutive reading errors allowed before aborting.",
                )}
                ${this.numberField(
                  "max_nudges",
                  "Stale-reading nudges",
                  defaults.max_nudges,
                  0,
                  20,
                  "1",
                  "Temporarily changes a light when its power sensor stops updating. Keep at 0 unless needed.",
                )}
              </div>
            </section>
            ...
            <section class="settings-section" ?hidden=${this.activeSection !== "github"} aria-labelledby="github-title">
              ...
              <p class="muted">
                Connect GitHub once to open profile-library pull requests from completed measurements.
              </p>
  ...
  disconnectedCallback(): void {
    ...
    clearTimeout(this.ocrPreviewDebounce);
    // Do not emit ocr-preview-stop here. Leaving Defaults to go back to setup/review
    // used to kill the camera the next screen still needs; the controller owns that
    // lifetime now (keeps it on setup/review, stops it on running/sessions/result).
  }

  /** Debounce a (re)start of the camera preview after a relevant field changes. A no-op
   * form edit (e.g. a witness's tolerance) also goes through this harmlessly -- the
   * backend simply reports the same, or still no, OCR component either way. */
  private scheduleOcrPreview(): void {
    clearTimeout(this.ocrPreviewDebounce);
    this.ocrPreviewDebounce = setTimeout(() => {
      const settings = this.collect();
      if (settings) emit<AppSettingsUpdate>(this, "ocr-preview-start", settings);
    }, OCR_PREVIEW_DEBOUNCE_MS);
  }

  private renderOcrPreview() {
    const previewId = this.meterPreviewId;
    if (!previewId || !this.meterPreviewLabels.length) return nothing;
    return html`
      <div class="section-fields ocr-preview-group">
        <h4>Camera preview</h4>
        <p class="muted">Aim the camera so the meter's display is fully inside the frame.</p>
        ${this.meterPreviewLabels.map(
          (label) =>
            html`<measure-ocr-preview kind="preview" sessionId=${previewId} label=${label}></measure-ocr-preview>`,
        )}
      </div>
    `;
  }
  ...
  private renderMeterFields(type: PowerMeterType) {
    const fields: Record<PowerMeterType, () => unknown> = {
      ...
      mystrom: () =>
        this.renderAddressField("mystrom_ip", "myStrom address (IP or hostname)", this.settings?.mystrom_ip),
      tasmota: () =>
        this.renderAddressField("tasmota_ip", "Tasmota address (IP or hostname)", this.settings?.tasmota_ip),
  ...
  private renderAddressField(name: string, label: string, value: string | null | undefined) {
    return html` <label>
      ...
      <input
        ...
        @input=${this.powerMeterSettingsChanged}
      />
      <small class="field-hint"
        >A hostname works too, as long as this machine can resolve it -- there's nothing IP-specific here, it's just
        passed straight to an HTTP request.</small
      >
    </label>`;
  }
  ...
  private renderTuyaFields() {
    return html` <div class="grid">
        <label>
          ...
          <input
            name="tuya_device_id"
            .value=${this.settings?.tuya_device_id ?? ""}
            required
            autocomplete="off"
            @input=${this.powerMeterSettingsChanged}
          />
        ...
        <label>
          <span>Tuya device address (IP or hostname)</span>
          <input
            name="tuya_device_ip"
            .value=${this.settings?.tuya_device_ip ?? ""}
            required
            autocomplete="off"
            placeholder="192.168.1.50 or device.local"
            @input=${this.powerMeterSettingsChanged}
      ...
      <label>
        ...
        <input
          name="tuya_version"
          .value=${this.settings?.tuya_version ?? "3.3"}
          autocomplete="off"
          @input=${this.powerMeterSettingsChanged}
        />
        <small class="field-hint"
          >Most devices from 2021 onward use 3.3 or 3.4; check the device's local key discovery output if readings
          fail.</small
        >
      </label>`;
  }
    ...
    return html` <div class="grid">
        ...
        <label>
          ...
          <input
            ...
            type="number"
            min="1"
            .value=${String(this.settings?.owon_baudrate ?? 9600)}
            required
            @input=${this.powerMeterSettingsChanged}
          />
        </label>
  ...
  private renderOcrFields() {
    return html` <label>
        ...
        <input
          name="ocr_source"
          .value=${this.settings?.ocr_source ?? "0"}
          autocomplete="off"
      ...
      <label>
        ...
        <small class="field-hint"
          >Names a display layout in measure/powermeter/ocr/layout.py describing where each reading appears on
          screen.</small
        >
      </label>`;
  }
  ...
  private renderTestRow() {
    return html` <div class="test-row">
      <button type="button" @click=${this.test} ?disabled=${this.testing || this.busy}>
        ${this.testing ? "Validating…" : "Validate measurement device"}
      </button>
    ...
    const permissionsHint =
      this.contributionAuth?.permissions_verified === false
        ? html`<span class="field-hint"
            >Identity verified. Fine-grained token permissions can only be confirmed during submission.</span
          >`
        : nothing;
    return html`
      <div class="identity">
        ...
        <button class="danger" type="button" @click=${this.disconnectGithub} ?disabled=${this.contributionAuthBusy}>
          Disconnect
        </button>
      </div>
    `;
  ...
  private renderGithubSection() {
    return html`
      <div class="section-fields">
        ...
        <p class="notice">
          GitHub credentials are stored by the measure app and can be included in Home Assistant backups. Disconnect
          GitHub before sharing or exporting backups you do not control.
        </p>
        ${this.contributionAuthError
          ? html`<p class="notice error" role="alert">${this.contributionAuthError}</p>`
          : nothing}
      </div>
    `;
  }
      ...
      <div class="device-flow">
        <div class="device-step">
          ...
          ${this.githubCopyStatus
            ? html`<span class="field-hint" role="status" aria-live="polite">${this.githubCopyStatus}</span>`
            : nothing}
        </div>
        <div class="device-step">
          ...
          <a
            class="github-link"
            href=${this.contributionDeviceFlow.verification_uri}
            target="_blank"
            rel="noopener noreferrer"
            >Continue on GitHub ↗</a
          >
        ...
        <span class="field-hint" role="status" aria-live="polite">
          ${status?.message ??
          "Waiting for GitHub authorization… This page will connect automatically when you finish."}
        </span>
      </div>
    `;
  }
        ...
        <label>
          ...
          <div class="token-row">
            <input
              ...
              autocomplete="off"
              placeholder="ghp_…"
              @keydown=${this.tokenKeydown}
            />
            <button type="button" @click=${this.saveGithubToken} ?disabled=${this.contributionAuthBusy}>
              Save token
            </button>
          </div>
  ...
  private renderShellyFields() {
    ...
    return html` <div class="discovery">
      ...
      <label>
        <span>Shelly address (IP or hostname)</span>
        <input
          name="shelly_ip"
          .value=${address}
          required
          autocomplete="off"
          placeholder="192.168.1.50 or shelly.local"
          @input=${this.shellyIpChanged}
        />
        <small class="field-hint"
          >Select a discovered device above, or enter its address manually -- a hostname works too, not just an
          IP.</small
        >
      </label>
      <div class="grid">
        <label>
          ...
          <input
            name="shelly_username"
            .value=${this.shellyUsername ?? this.settings?.shelly_username ?? DEFAULT_SHELLY_USERNAME}
            required
            autocomplete="username"
            maxlength="50"
            @input=${this.shellyUsernameChanged}
          />
          <small class="field-hint"
            >Only used if the device's own "Restrict login" is on. Gen1 devices may use a custom username; Gen2 and
            newer always use admin regardless of this field.</small
          >
        </label>
        <label>
          <span>Shelly password</span>
          <input
            name="shelly_password"
            type="password"
            .value=${this.shellyPassword}
            autocomplete="new-password"
            maxlength="255"
            placeholder=${this.settings?.shelly_password_configured
              ? "Saved password (leave blank to keep)"
              : "Optional"}
            @input=${this.shellyPasswordChanged}
          />
          <small class="field-hint"
            >Leave blank unless the device's own "Restrict login" is on. Stored privately in the app and never returned
            by the API.</small
          >
        </label>
      </div>
  ...
  private renderClearShellyPassword() {
    ...
    return html` <label class="check">
      <input
        name="clear_shelly_password"
        type="checkbox"
        .checked=${this.clearShellyPassword}
        @change=${this.clearShellyPasswordChanged}
      />
  ...
  private renderShellyDiscovery(selectedAddress: string) {
    if (this.discoveringShellys)
      return html`<p class="discovery-status" role="status">Searching for Shelly devices on your network…</p>`;
    if (this.shellyDiscoveryError)
      return html`<p class="discovery-status error" role="alert">${this.shellyDiscoveryError}</p>`;
    if (this.shellyDiscoveryAvailable === false) {
      return html`<p class="discovery-status">
        ${this.shellyDiscoveryMessage ?? "Shelly discovery is unavailable. Enter the IP address manually."}
      </p>`;
    }
    if (!this.shellyDiscoveryDevices.length)
      return html`<p class="discovery-status">
        No Shelly devices found. You can refresh or enter an IP address manually.
      </p>`;
    return optionSelect(
      "discovered_shelly",
      "Select device",
  ...
  private renderKasaFields() {
    ...
    return html` <label>
      <span>Kasa address (IP or hostname)</span>
      <input
        name="kasa_ip"
        .value=${address}
        required
        autocomplete="off"
        placeholder="192.168.1.50 or kasa-plug.local"
        @input=${this.kasaIpChanged}
      />
      <small class="field-hint"
        >Enter the address of a Kasa plug with energy monitoring, such as a KP115 or HS110 -- a hostname works too. Give
        it a static lease/reservation in your router (or DNS entry) so it stays reachable.</small
      >
    </label>`;
  }
  ...
  private shellyDeviceLabel(device: ShellyDiscoveryDevice): string {
    const identity = [
      device.name,
      device.model,
      device.generation === null ? null : `Gen ${device.generation}`,
      device.ip_address,
    ]
  ...
  private renderWitnessRow(witness: WitnessSettings, index: number) {
    return html` <div class="witness-row">
      ...
      <div class="grid">
        <label>
          ...
          <input
            ...
            @input=${(event: Event) =>
              this.witnessFieldChanged(index, "offset_w", (event.currentTarget as HTMLInputElement).value)}
          />
          <small class="field-hint"
        ...
        <label>
          ...
          <input
            ...
            step="0.01"
            min="0"
            .value=${String(witness.tolerance_w)}
            @input=${(event: Event) =>
              this.witnessFieldChanged(index, "tolerance_w", (event.currentTarget as HTMLInputElement).value)}
          />
        </label>
        <label>
          <span>Tolerance (%)</span>
          <input
            ...
            step="0.1"
            min="0"
            .value=${String(witness.tolerance_pct)}
            @input=${(event: Event) =>
              this.witnessFieldChanged(index, "tolerance_pct", (event.currentTarget as HTMLInputElement).value)}
          />
          <small class="field-hint"
            >Whichever of the two tolerances is larger, for the current reading, is the one applied.</small
          >
        </label>
      </div>
      <label class="check">
        <input
          type="checkbox"
          .checked=${witness.required}
          @change=${(event: Event) => this.witnessRequiredChanged(index, event)}
        />
    ...
    switch (meter.type) {
      ...
      case "shelly":
        return html` <label>
          <span>Shelly address (IP or hostname)</span>
          <input
            .value=${meter.device_ip ?? ""}
            required
            autocomplete="off"
            placeholder="192.168.1.51 or shelly.local"
            @input=${set("device_ip")}
          />
        </label>`;
      case "kasa":
      ...
      case "tasmota":
        return html` <label>
          <span>Address (IP or hostname)</span>
          <input
            .value=${meter.device_ip ?? ""}
            required
            autocomplete="off"
            placeholder="192.168.1.51 or device.local"
            @input=${set("device_ip")}
          />
        </label>`;
      case "tuya":
        return html` <div class="grid">
          <label
            ><span>Tuya device ID</span
            ><input .value=${meter.device_id ?? ""} required autocomplete="off" @input=${set("device_id")}
          /></label>
          <label
            ><span>Tuya device address (IP or hostname)</span
            ><input .value=${meter.device_ip ?? ""} required autocomplete="off" @input=${set("device_ip")}
          /></label>
        </div>`;
      case "owh98xx":
        return html` <div class="grid">
          ...
          <label
            ><span>Baud rate</span
            ><input type="number" min="1" .value=${String(meter.baudrate ?? 9600)} required @input=${set("baudrate")}
          /></label>
        </div>`;
      case "ocr":
        return html` <label>

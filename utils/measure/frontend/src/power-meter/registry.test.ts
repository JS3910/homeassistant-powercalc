...
describe("power meter registry", () => {
  ...
  it("mounts an OCR preview from the request spec before the session API answers", () => {
    expect(ocrPreviewLabelsFromSpec({ type: "ocr", source: "0" })).toEqual(["primary"]);
    expect(
      ocrPreviewLabelsFromSpec({
        type: "composite",
        primary: { type: "ocr", source: "0" },
        witnesses: [{ meter: { type: "shelly", device_ip: "192.0.2.20" } }],
      }),
    ).toEqual(["primary"]);
    expect(ocrPreviewLabelsFromSpec({ type: "shelly", device_ip: "192.0.2.20" })).toEqual([]);
  });

...
Each accepted meter read is one JSON line, keyed by the same mode and variation as the
LUT row that will store their mean. A bare single meter still writes a ``primary.power``
so reading count is evidence, not a session setting. ``write`` is a no-op only when
there is neither a reading, a power value, nor settle data to record.
"""
...
class RawSampleWriter:
    ...
    def write(
        ...
        *,
        ...
        if reading is None and settle_seconds is None and power is None:
            return
        row: dict = {
            "timestamp": time.time(),
            "mode": mode.value,
        ...
        if reading is not None:
            ...
            row["primary"] = {"power": power, "voltage": None, "current": None, "power_factor": None}
        try:

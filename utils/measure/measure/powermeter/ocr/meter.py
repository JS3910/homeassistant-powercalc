...
from measure.powermeter.errors import OutdatedMeasurementError, PowerMeterError, WaitingForMeterError
...
class OcrPowerMeter(PowerMeter):
    ...
    Unreadable frames raise ``WaitingForMeterError`` so the run waits; they can clear on
    the next frame. ``OutdatedMeasurementError`` is only for a camera that stays silent
    after a reconnect. The display is re-located when readings keep failing or the
    picture changes a lot, both signs the meter or camera moved.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._silence_recovered_at: float | None = None
    ...
    @property
    def preview(self) -> PreviewServer | None:
        """The in-process preview publisher, if one was configured for this meter."""
    ...
    def recover(self) -> None:
        """Forget the display location, drop stale accepts, and reopen the camera."""
        _LOGGER.warning("Recovering OCR meter: forgetting display location and reconnecting the camera")
        self._reader.invalidate()
        with self._lock:
            self._accepted.clear()
            self._last = None
            self._consecutive_rejections = 0
            self._silence_recovered_at = self._clock()
        recover = getattr(self._frames, "recover", None)
        if callable(recover):
            recover()
    ...
    def get_power(self, include_voltage: bool = False) -> PowerMeasurementResult:
        ...
        with self._lock:
            ...
            last = self._last
            recovered_at = self._silence_recovered_at
        if not recent:
            if newest is not None and now - newest.timestamp <= self._stale_after_seconds:
                recent = [newest]
            elif self._frames_live(now, last):
                raise WaitingForMeterError(self._waiting_reason(newest, last))
            elif recovered_at is None and last is None and now - self._started_at <= self._stale_after_seconds:
                raise WaitingForMeterError("OCR: waiting for the first camera frame")
            elif recovered_at is None:
                self.recover()
                raise WaitingForMeterError(self._reconnect_reason(last))
            elif now - recovered_at <= self._stale_after_seconds:
                raise WaitingForMeterError("OCR: waiting for camera after reconnect")
            else:
    ...
    def process(self, frame: Frame) -> DisplayReading:
        ...
        with self._lock:
            self._last = reading
            self._silence_recovered_at = None
    ...
    def _frames_live(self, now: float, last: DisplayReading | None) -> bool:
        if getattr(self._frames, "error", None):
            return False
        return last is not None and now - last.timestamp <= self._stale_after_seconds

    def _waiting_reason(self, newest: DisplayReading | None, last: DisplayReading | None) -> str:
        if last is None:
            return "OCR: waiting for the first camera frame"
        return f"OCR: waiting for a readable frame; last frame: {last.reason or 'ok'}"

    def _reconnect_reason(self, last: DisplayReading | None) -> str:
        if last is None:
            return "OCR: camera silent, reconnecting"
        return f"OCR: camera silent, reconnecting; last frame: {last.reason or 'ok'}"
    ...
    def _stale_reason(self, now: float, newest: DisplayReading | None, last: DisplayReading | None) -> str:
        ...
        if newest is None:
            return f"OCR: no accepted reading yet; last frame: {last.reason or 'ok'}"

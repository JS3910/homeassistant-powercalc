...
class FrameSource:
    ...
    def recover(self) -> None:
        """Drop the current capture so the background thread reopens the source."""
        self._reconnect.set()
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if self._reconnect.is_set():
                _LOGGER.info("Video source %r reconnect requested", self._source)
                continue
    ...
    def _backoff(self, delay: float) -> None:
        """Wait for ``delay`` seconds, but wake early if stop or recover is requested."""
    ...
    def _read_until_failure(self, capture: VideoCapture) -> None:
        """Read until the source fails repeatedly, goes silent, or recover is requested."""
        failures = 0

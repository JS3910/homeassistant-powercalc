...
def ocr_preview_binds(spec: PowerMeterSpec) -> frozenset[tuple[str, int]]:
    """Listen addresses an OCR aiming-preview HTTP server would bind for this spec."""

    binds: set[tuple[str, int]] = set()

    def consider(meter: object) -> None:
        if isinstance(meter, OcrPowerMeterSpec) and meter.preview_port is not None:
            binds.add((meter.preview_host, meter.preview_port))
        elif isinstance(meter, CompositePowerMeterSpec):
            consider(meter.primary)
            for witness in meter.witnesses:
                consider(witness.meter)

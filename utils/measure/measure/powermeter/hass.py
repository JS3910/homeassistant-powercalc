...
class HassPowerMeter(PowerMeter):
    ...
    def recover(self) -> None:
        """Ask Home Assistant to refresh the sensor instead of flashing the light."""
        if not self._entity_id:
            return
        self.client.trigger_service("homeassistant", "update_entity", entity_id=self._entity_id)
        if self._voltage_entity_id:
            self.client.trigger_service("homeassistant", "update_entity", entity_id=self._voltage_entity_id)
